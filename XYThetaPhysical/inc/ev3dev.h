...
public:
  ...
  static constexpr char ht_color[]       = "ht-nxt-color-v2";
...
class ht_color_sensor : public sensor
{
public:
  ht_color_sensor(address_type address = INPUT_AUTO);

  static constexpr char mode_col_color[] = "COLOR";
  static constexpr char mode_col_norm[] = "NORM";
  static constexpr char mode_col_raw[] = "RAW";
  static constexpr char mode_col_all[] = "ALL";
  static constexpr char mode_col_passive[] = "PASSIVE";
  ...
  std::tuple<int, int, int, int> normalized_colors(bool do_set_mode = true) {
    if (do_set_mode) set_mode(mode_col_norm);
    return std::make_tuple(value(0), value(1), value(2), value(3));
  }

  std::tuple<int, int, int, int> raw_colors(bool do_set_mode = true) {
    if (do_set_mode) set_mode(mode_col_raw);
    return std::make_tuple(value(0), value(1), value(2), value(3));
  }

  std::tuple<int, int, int, int, int> all_colors(bool do_set_mode = true) {
    if (do_set_mode) set_mode(mode_col_all);
    return std::make_tuple(value(0), value(1), value(2), value(3), value(4));
  }

  std::tuple<int, int, int, int> passive_colors(bool do_set_mode = true) {
    if (do_set_mode) set_mode(mode_col_passive);
    return std::make_tuple(value(0), value(1), value(2), value(3));
  }
};
